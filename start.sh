#!/bin/bash
# set -ex # Temporarily disable exit on error for testing

echo "--- start.sh: MINIMAL VERSION ALIVE ---"

# Keep the container alive for a long time to see if it logs *anything*
sleep 3600 

echo "--- start.sh: MINIMAL VERSION SLEPT, NOW EXITING --- " # Should not be reached in normal test
exit 0
